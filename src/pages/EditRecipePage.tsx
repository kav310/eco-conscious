...
const EditRecipePage = () => {
    ...
    const [recipeData, setRecipeData] = useState<RecipesResponse | any>(undefined);
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            const response = await axios.put(`http://localhost:5000/editRecipe/${id}`, recipeData, {headers});
    ...
    useEffect(() => {
        const separator = ", ";
        const ingredients = recipeData && recipeData.ingredients.join(separator);
        if (recipeData) {
            setValue("name", recipeData.name);
            setValue("description", recipeData.description);
            setValue("ingredients", ingredients);
            setValue("cookingInstructions", recipeData.cookingInstructions);
